...
const RegisterForm = ({ walletAddress, referredBy }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Step 1: Approve tokens (uncomment if needed)
      const approveResult = await approveTokens(signer, approvalAmount);
      console.log("Approval Result:", approveResult);

      console.log(data)
      const {fullName , referredBy} = data

      // const approveResultContract = await approveTokens(registrationContractAddress, approvalAmount);
      // console.log("approveResultContract Result:", approveResultContract);
